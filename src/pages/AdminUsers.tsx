...
export default function AdminUsers() {
  ...
  const onToggleBlock = async (id: number, faol: boolean) => {
    const action = faol ? 'заблокировать' : 'активировать'
    if (!confirm(`Вы хотите ${action} пользователя?`)) return
    setBusy(id)
    try {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">
          Блокировка, разрешения и просмотр профиля
        </p>
      </div>
      ...
      <div className="grid gap-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-gray-100 sm:grid-cols-3">
        ...
        <div>
          ...
          >
            ...
            <option value="qariya">Пожилой</option>
            <option value="voluntyor">Волонтер</option>
